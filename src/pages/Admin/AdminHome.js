...
const renderActiveShape = (props) => {
  ...
  const { cx, cy, midAngle, innerRadius, outerRadius, startAngle, endAngle, fill, payload, percent, amount } = props;
  ...
  return (
    <g>
      ...
      <text x={ex + (cos >= 0 ? 1 : -1) * 12} y={ey} textAnchor={textAnchor} fill="#333">{`Số lượng ${amount}`}</text>
...
const AdminHome = () => {
  ...
  let processDate = {};
  console.log("start: ", dayStart, "end: ", dayEnd);
  ...
  useEffect(() => {
    if (dayStart !== undefined && dayEnd !== undefined) {
      const formattedStartDate = dayStart.toISOString().split("T")[0];
      const formattedEndDate = dayEnd.toISOString().split("T")[0];
      processDate = {
  ...
  return <>
    {/* <CategoryAmount /> */}
    <div className="container d-flex mt-5" style={{ height: "60vh", justifyContent: "space-between" }}>
